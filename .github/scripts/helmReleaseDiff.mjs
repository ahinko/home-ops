...
async function helmTemplate (releaseName, registryName, chartName, chartVersion, chartValues) {
  ...
  const manifests = await $`${helm} template --kube-version 1.26.1 --release-name ${releaseName} --include-crds=false ${registryName}/${chartName} --version ${chartVersion} --values ${valuesFile.stdout.trim()}`
