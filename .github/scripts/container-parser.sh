...
entry() {
    ...
    # some helm charts uses image instead of repository (ingress-nginx as an example)
    images+=("$(yq eval-all '[.. | select(has("image")) | select(has("tag"))] | .[] | .image + ":" + .tag' "${file}" 2>/dev/null)")

    # look for init containers
    images+=("$(yq eval-all '.spec.values.*.initContainers.[].image' "${file}" 2>/dev/null)")

    # look for extra init containers (example ingress-nginx)
    images+=("$(yq eval-all '.spec.values.*.extraInitContainers.[].image' "${file}" 2>/dev/null)")
